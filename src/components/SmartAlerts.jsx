  ...
  try {
    ...
    if (sim) {
      ...
      if (regime === 'mei' && receitaAnual > 50000 && receitaAnual < 81000) {
        ...
        alerts.push({
          ...
          message: `A receita anual da empresa (${formatCurrency(receitaAnual)}) está a ${((81000 - receitaAnual) / 81000 * 100).toFixed(0)}% do limite MEI. Planeje a transição para Simples Nacional.`,
          action: 'comparativo',
      ...
      if (regime === 'simples' && receitaAnual > 1000000) {
        alerts.push({
          ...
          message: `Com receita de ${formatCurrency(receitaAnual)}/ano no Simples, vale comparar com Lucro Presumido. Empresas com margem alta podem economizar até 30% em impostos.`,
          action: 'comparativo',
      ...
      if (receitaMensal > 0) {
        ...
        if (imposto > 0 && (imposto / receitaMensal) > 0.25) {
          alerts.push({
            ...
            message: `Os impostos representam ${((imposto / receitaMensal) * 100).toFixed(1)}% da receita da empresa. Considere otimizar usando o Comparativo de Regimes.`,
            action: 'comparativo',
    ...
    if (dre) {
      ...
      if (rb > 0 && lucro < 0) {
        alerts.push({
          ...
          message: `O demonstrativo mostra prejuízo de ${formatCurrency(Math.abs(lucro))}. Revise custos e preços dos produtos urgentemente.`,
          action: 'precificacao',
      ...
      if (rb > 0 && lucro > 0 && (lucro / rb) < 0.05) {
        alerts.push({
          ...
          message: `A margem líquida da empresa é de apenas ${((lucro / rb) * 100).toFixed(1)}%. Considere revisar os preços dos produtos ou reduzir custos.`,
          action: 'precificacao',
    ...
    if (!custos && sim) {
      alerts.push({
        ...
        type: 'info',
        icon: Lightbulb,
        title: 'Configure os custos da empresa',
        message: 'Cadastre os custos operacionais da empresa para uma precificação mais precisa e relatórios completos.',
        action: 'custos',
