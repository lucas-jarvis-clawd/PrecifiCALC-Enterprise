...
export default function Precificacao() {
  ...
  // Calcula alíquota efetiva com base no regime e receita
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="border-b border-slate-200 pb-4">
        ...
        <p className="text-slate-500 text-sm mt-1">Calcule o preço de venda considerando custos, impostos e margem desejada</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Parâmetros */}
        <Card>
          <CardHeader><h2 className="text-slate-800 font-medium text-sm">Parâmetros do Produto/Serviço</h2></CardHeader>
          <CardBody className="space-y-3">
            ...
            <InputField label={tipo === 'produto' ? 'Custo do Produto (unitário)' : 'Custo do Serviço (unitário)'} value={custoProduto} onChange={setCustoProduto} prefix="R$" step={10} />
            <InputField label="Despesas Fixas Mensais" value={despesasFixas} onChange={setDespesasFixas} prefix="R$" step={500} help="Aluguel, salários, etc." />
            <InputField label="Despesas Variáveis (% do custo)" value={despesasVariaveisPercent} onChange={setDespesasVariaveisPercent} suffix="%" step={1} help="Comissões, frete, etc." />
            ...
            <div className="border-t border-slate-200 pt-3 mt-3">
              ...
              {regime === 'simples' && (
                <>
                  ...
                  <InputField label="Receita Bruta 12 Meses (RBT12)" value={rbt12} onChange={setRbt12} prefix="R$" step={10000} className="mt-3" help={`Alíquota efetiva: ${formatPercent(aliquotaEfetiva)}`} />
                </>
              )}
              {regime !== 'simples' && (
                <InputField label="Receita Mensal Estimada" value={receitaMensal} onChange={setReceitaMensal} prefix="R$" step={5000} className="mt-3" help={`Alíquota efetiva: ${formatPercent(aliquotaEfetiva)}`} />
              )}
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <StatCard icon={Calculator} label="Ponto de Equilíbrio" value={`${calculo.pontoEquilibrioQtd} un.`} subvalue={formatCurrency(calculo.pontoEquilibrioReceita)} color="amber" />
          </div>

          {/* Composição do preço */}
          <Card>
            <CardHeader><h2 className="text-slate-800 font-medium text-sm">Composição do Preço de Venda</h2></CardHeader>
            <CardBody>
              <div className="space-y-2">
                <PriceRow label="Custo unitário" value={calculo.custoUnitario} total={calculo.precoVenda} color="bg-red-500" />
                <PriceRow label="Despesas variáveis" value={calculo.despVariavelUnitario} total={calculo.precoVenda} color="bg-amber-500" />
                ...
                <div className="border-t border-slate-200 pt-2 flex justify-between">
                  <span className="text-sm font-medium text-slate-800">Preço de Venda</span>
              ...
              <div className="mt-4 p-3 bg-slate-50 rounded-md">
                <p className="text-xs text-slate-500">
                  <span className="text-slate-700 font-medium">Fórmula:</span> Preço = Custo Total / (1 - Alíquota Efetiva - Margem Desejada)
                </p>
          ...
          {/* Projeção mensal */}
          <Card>
            <CardHeader><h2 className="text-slate-800 font-medium text-sm">Projeção Mensal ({quantidadeMensal} unidades)</h2></CardHeader>
            <CardBody>
          ...
          {/* Sensibilidade */}
          <Card>
            <CardHeader><h2 className="text-slate-800 font-medium text-sm">Sensibilidade: Preço x Margem</h2></CardHeader>
            <CardBody>
