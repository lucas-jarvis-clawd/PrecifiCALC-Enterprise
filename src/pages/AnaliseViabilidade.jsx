...
import {
  calcMEI, calcSimplesTax, calcLucroPresumido, calcLucroReal,
...
export default function AnaliseViabilidade() {
  ...
  const calcularViabilidade = () => {
    ...
    // Calculo tributario REAL usando funcoes do taxData
    let impostos = 0;
    ...
    switch (dados.regime) {
      ...
      case 'simples': {
        const r = calcSimplesTax(receitaAnual, dados.anexo);
        impostos = r && !r.excedeLimite ? r.valorMensal : 0;
        break;
      }
      case 'presumido': {
        const r = calcLucroPresumido(receita, dados.atividade, (parseFloat(dados.issAliquota) || 5) / 100);
        impostos = r && !r.erro ? r.totalMensal : 0;
        break;
      }
      case 'real': {
        const desp = parseFloat(dados.despesasDedutiveis) || custoFixo;
        const cred = parseFloat(dados.creditosPisCofins) || 0;
        const r = calcLucroReal(receita, desp, cred, (parseFloat(dados.issAliquota) || 5) / 100);
        impostos = r && !r.erro ? r.totalMensal : 0;
        break;
      }
    }
    ...
    // Ponto de equilibrio: receita onde lucro = 0
    // custoFixo + impostos_fixos = receita * (1 - custoVariavel% - aliquota_efetiva%)
    const aliquotaEfetiva = receita > 0 ? impostos / receita : 0;
    const custoVariavelPercDecimal = (parseFloat(dados.custoVariavelPercent) || 0) / 100;
    const margemContribuicao = 1 - custoVariavelPercDecimal - aliquotaEfetiva;
    ...
    setResultado({
      lucroMensal, margemLucro, payback, pontoEquilibrio,
      impostos, aliquotaEfetiva, custoTotal, projecao, distribuicao,
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="border-b border-surface-700 pb-4">
        ...
        <p className="text-surface-400 text-sm mt-1">Avalie a viabilidade do negocio com calculos tributarios reais</p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <Card>
          <div className="px-5 py-4 border-b border-surface-700">
            <h2 className="font-medium text-white text-sm">Dados do Negocio</h2>
          </div>
          <CardBody className="space-y-3">
            ...
            <button
              ...
              className="w-full px-4 py-2.5 bg-brand-600 text-white rounded-md font-medium hover:bg-brand-700 transition-colors text-sm"
            >
          ...
          {resultado ? (
            <>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                <StatCard
                  icon={resultado.viabilidade === 'inviavel' || resultado.viabilidade === 'limitada' ? AlertCircle : CheckCircle}
                  label="Viabilidade"
                  value={viabTexto[resultado.viabilidade]}
                  color={viabColor[resultado.viabilidade]}
                />
                ...
                <StatCard icon={Target} label="Ponto Equilibrio" value={formatCurrency(resultado.pontoEquilibrio)} subvalue="Receita minima" color="amber" />
              </div>
              ...
              <Card>
                <CardBody>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div>
                      <p className="text-surface-500 text-xs">Impostos Mensais</p>
                      <p className="text-white font-medium">{formatCurrency(resultado.impostos)}</p>
                    </div>
                    <div>
                      <p className="text-surface-500 text-xs">Aliquota Efetiva</p>
                      <p className="text-white font-medium">{formatPercent(resultado.aliquotaEfetiva)}</p>
                    </div>
                    <div>
                      <p className="text-surface-500 text-xs">Custos Totais</p>
                      <p className="text-white font-medium">{formatCurrency(resultado.custoTotal)}</p>
                    </div>
                    <div>
                      <p className="text-surface-500 text-xs">Lucro Anual</p>
                      <p className={`font-medium ${resultado.lucroMensal > 0 ? 'text-emerald-400' : 'text-red-400'}`}>{formatCurrency(resultado.lucroMensal * 12)}</p>
                    </div>
                  </div>
                </CardBody>
              </Card>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <Card>
                  <div className="px-5 py-3 border-b border-surface-700">
                    <h3 className="font-medium text-white text-sm">Projecao 12 Meses</h3>
                  </div>
                  <CardBody>
            ...
            <Card className="flex items-center justify-center h-64">
              <div className="text-center">
                <Calculator size={40} className="text-surface-600 mx-auto mb-3" />
                <p className="text-surface-500 text-sm">Preencha os dados e clique em Analisar</p>
              </div>
            </Card>
          )}
        </div>
